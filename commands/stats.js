...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      async function getFollowerCount(username, follow) {
        try {
          const response = await axios.get(`https://scratch.mit.edu/users/${username}/${follow}/`, {
            headers: {
          ...
          // div.box-head の中のテキストを取得
          const boxHead = document.querySelector('.box-head');
          if (!boxHead) {
            console.log('box-head が見つかりませんでした');
            return;
          }

          const text = boxHead.textContent;
          const match = text.match(/\((\d+)\)/); // フォロワー (853) から数字を抽出
          if (match) {
      ...
      followers = await getFollowerCount(username_2, "followers");
      followings = await getFollowerCount(username_2, "following");
